...
impl BackgroundCompiler {
    pub fn spawn() -> (Self, Receiver<CompiledBlock>) {
        ...
        (Self { tx, join: Some(join) }, result_rx)
    }
...
impl Drop for BackgroundCompiler {
    fn drop(&mut self) {
        // Dropping the sender wakes the worker and lets it exit cleanly once
        // all queued requests have been drained.
